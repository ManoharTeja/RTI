'''
Trying to make rhoinit as a parameter instead of variable
'''
...
# Continuity equation
problem.add_equation("dt(rho) + v*dy(rhoinit) - (1/Re)*(dx(dx(rho)) + dy(rhoy)) = - u*dx(rho) - v*(rhoy)")
# x-momentum eqn
problem.add_equation("dt(u) + dx(p) - (1/Re)*(dx(dx(u)) + dy(uy)) = - u*dx(u) - v*uy - (p/(rho+rhoinit))*dx(rho)")
# y-momentum eqn
problem.add_equation("dt(v) + dy(p) - (1/Re)*(dx(dx(v)) + dy(vy)) = - u*dx(v) - v*vy - (p/(rho+rhoinit))*(rhoy+dy(rhoinit)) - (rho*gr)/(rho+rhoinit)")
# ************************************************************
...
analysis.add_task('(rho+rhoinit)')
analysis.add_task('rho')
analysis.add_task('u')
analysis.add_task('v')
solver.evaluator.vars['Lx'] = Lx
analysis.add_task("integ((rho+rhoinit),'x')/Lx", name='integral rho_x')
analysis.add_task("integ((rho+rhoinit),'x')/Lx", name='integral rhofluc_x')
